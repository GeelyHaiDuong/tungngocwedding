(function () {
  "use strict";
  console.info("[Wedding] Family cache fix V20 loaded");
  const C = window.WEDDING_CONFIG || {};
  const FAMILY_ASSET_TOKEN = Date.now().toString(36);
  ...
  function resolveImage(src) {
    ...
    try {
      ...
      if (u.origin === location.origin) {
        u.searchParams.set("v", FAMILY_ASSET_TOKEN);
      }
      return u.href;
    } catch (e) {
      return String(src);
  ...
  function splitParentLines(parents) {
    let text = String(parents || "").trim();
    if (!text) return [];
    ...
    text = text
      ...
      .replace(/&amp;/gi, "&")
      .replace(/\r/g, "")
      .replace(/\s*(?:-|–|—|;|\||&|\bvà\b)\s*(?=(?:Ông|Bà|Cha|Mẹ)\b)/gi, "\n")
      .replace(/\s+(?=(?:Ông|Bà|Cha|Mẹ)\s*:)/gi, "\n");
  ...
  function familyBlock(side, parents) {
    const lines = splitParentLines(parents);

    if (!lines.length) {
      return `<strong>${side}</strong><br><span>Thông tin bố mẹ</span>`;
    }

    return `<strong>${side}</strong><br>${lines.map(escapeHtml).join("<br>")}`;
  }
  ...
  function applyInfo() {
    ...
    setHtml("HEADLINE19", familyBlock("NHÀ TRAI", fm.groomParents));
    setHtml("HEADLINE20", familyBlock("NHÀ GÁI", fm.brideParents));
